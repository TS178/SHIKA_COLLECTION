/* guide.js — 画面を初めて開いたときに1回だけ出す案内。
   ...
   使い方: showGuide('mapGuideShown', { title, lines, icon: 'pin' | 'star' })
 ...
 */
export function showGuide(flag, { title, lines = [], icon = 'pin' } = {}) {
  ...
  const attempt = () => {
    ...
    card.append(el('div', { class: 'guide__tap', text: 'タップして閉じる' }));

    const guide = el('div', { class: 'guide' }, [card]);
    const close = () => {
